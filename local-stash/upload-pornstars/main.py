...
- Country: takes FIRST nationality, converts "America" → "United States of America"
- Measurements: Bra–Waist–Hip (Bust ignored)
- Fake Tits: from "Boobs" field
- Height (feet/in → cm), Weight (kg)
...
def process_performer(name, data):
    log(f"→ Importing performer: {name}")
    ...
    if existing:
        ...
        if res and res.get("data") and not res.get("errors"):
            log(f"   ✅ Updated performer: {name}", "success")
            return True
        else:
        ...
        if res and res.get("data") and not res.get("errors"):
            log(f"   ✅ Created performer: {name}", "success")
            return True
        else:
            log(f"   ❌ Create failed: {res}", "error")
            return False
...
def main():
    ...
    log(f"🎉 DONE — imported {count} performers.", "success")
