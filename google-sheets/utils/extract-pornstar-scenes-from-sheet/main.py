...
DATA_DIR = (
    BASE_DIR / "google-sheets" / "utils" / "extract-pornstar-scenes-from-sheet" / "data"
)
...
def is_magenta_text(col_index: int) -> bool:
    ...
    text_fmt = cell.get("effectiveFormat", {}).get("textFormat", {})

    rgb = text_fmt.get("foregroundColorStyle", {}).get("rgbColor")
        ...
        for data in sheet["data"]:
            ...
            for r, row in enumerate(data.get("rowData", [])):
                ...
                for c, cell in enumerate(row.get("values", [])):
                    ...
                    if col in (
                        COL_PORNSTAR,
                        COL_NETWORK_OR_STUDIO,
                        COL_SITE_OR_WEBSERIE,
                    ):
                        rows[r][col] = cell
